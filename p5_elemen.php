            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="#">Elemen</a>
                        </li>
                        <li class="nav-item">
                        ...
                        <div class="card mb-4">
                            <div class="card-header" style="font-weight: bold;">
                                ...
                                Elemen Project untuk Setiap Dimensi
                            </div>
                            <div class="card-body">
                            
                                <table class="table table-bordered" style="width: 75%">
                                    <?php 
                                    $queryElemen = mysqli_query($conn, "SELECT 
                                    `id_elemen`, pe.id_dimensi, `dimensi`,`elemen` 
                                    FROM `p5_elemen` pe
                                    LEFT JOIN p5_dimensi pd ON pe.id_dimensi = pd.id_dimensi
                                    ORDER BY pe.id_dimensi;");

                                    $groupedElements = [];

                                    while ($row = mysqli_fetch_assoc($queryElemen)) {
                                        $idDimensi = $row['id_dimensi'];
                                        $dimensi = $row['dimensi'];
                                        $id_elemen = $row['dimensi'];
                                        $elemen = $row['elemen'];

                                        // Group elements by dimension
                                        if (!isset($groupedElements[$idDimensi])) {
                                            $groupedElements[$idDimensi] = ['dimensi' => $dimensi, 'elements' => []];
                                        }

                                        // Add element to the group
                                        $groupedElements[$idDimensi]['elements'][] = $elemen;
                                    }
                                    ...
                                    foreach ($groupedElements as $group) {
                                        $dimensi = $group['dimensi'];
                                        $elements = $group['elements'];

                                        echo '<h5>' . $h . '. Dimensi ' . $dimensi . ' :</h5>';
                                        echo '<table class="table table-bordered" style=" width: 75%">';
                                        echo '<tr>
                                                <th style="text-align: center; width: 5%">No.</th>
                                                <th style="text-align: left; width: 65%">Elemen</th>
                                              </tr>';
                                        ...
                                        foreach ($elements as $element) {
                                            echo '<tr>
                                                    ...
                                                    <td style="width: 65%">' . ucfirst($element) . '</td>
                                                  </tr>';
                                        }
